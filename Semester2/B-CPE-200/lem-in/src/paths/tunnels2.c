/*
** EPITECH PROJECT, 2022
** tunnels2.c
** File description:
** sees if a tunnel is duplicated before adding it
*/

#include "lemin.h"
#include "structs.h"

static int has_char(char *str, char c)
{
    int i = 0;

    for (; str[i] && str[i] != c; i++);
    return (str[i] == c);
}

tunnel_t *search_node(char *name, tunnel_t **list)
{
    int i;

    for (i = 0; list[i] != NULL && my_strcmp(name, list[i]->name) != 0; i++);
    return (list[i]);
}

tunnel_t **add_space(tunnel_t **tunnel, int nb)
{
    tunnel_t **new = malloc(sizeof(tunnel_t *) * (nb + 1));

    if (tunnel == NULL) {
        new[nb - 1] = malloc(sizeof(tunnel_t));
        new[nb] = 0;
        return (new);
    }
    for (int i = 0; i < nb - 1; i++)
        new[i] = tunnel[i];
    new[nb - 1] = malloc(sizeof(tunnel_t));
    new[nb] = NULL;
    free(tunnel);
    return (new);
}

void add_tunnel(tunnel_t ***tunnel, vector2_t pos, char *name, int nb)
{
    tunnel_t **lm_tunnel;

    my_strlen(name);
    *tunnel = add_space(*tunnel, nb);
    lm_tunnel = *tunnel;
    lm_tunnel[nb - 1]->tunnel_id = nb - 1;
    lm_tunnel[nb - 1]->x = pos.x;
    lm_tunnel[nb - 1]->y = pos.y;
    lm_tunnel[nb - 1]->name = my_strdup(name);
    lm_tunnel[nb - 1]->link_nb = 0;
    lm_tunnel[nb - 1]->links = 0;
    my_strlen(name);
}